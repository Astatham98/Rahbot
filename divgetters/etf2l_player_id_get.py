...
class Etf2l:
    def get_div(self, etf2l, sixes=True):
        ...
        response = requests.get(
            "https://api.etf2l.org/player/{}/results.json?per_page=100&since=0".format(
                id
            )
        )
        ...
        for gamemode in gamemodes:
            if response.status_code == 200:
                ...
                try:
                    for c in competitions:
                        ...
                        if (
                            comp["type"] == gamemode
                            and c["merced"] == 0
                            and "preseason" not in comp["name"].lower()
                        ):
                ...
                div = (
                    "".join(divs_split[:-1])
                    if divs_split[-1] in ("A", "B")
                    else "".join(divs_split)
                )

                div = (
                    "".join(div.split(" ")[:-1])
                    if div.split(" ")[-1] == "Division"
                    else div
                )
    ...
    def get_steam(etf2l):
        ...
        id = split[-2]
        response = requests.get(
            "https://api.etf2l.org/player/{}.json?per_page=100&since=0".format(id)
        )
        if response.status_code == 200:
            json_format = response.json()
