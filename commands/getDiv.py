...
class Div(BaseCommand):
    ...
    async def handle(self, params, msg, client):
        ...
        if no_users or exist_and_same:
            ...
            await self.give_role(
                msg, roles, divs
            )  # Give the user a new role and remove newb role
            await self.purge_and_post(
                msg
            )  # Purge the messages in the channel and post a new embed
        else:
    ...
    async def welcome_message_embed(self, message):
        embed = discord.Embed(
            ...
            description="Type ;div {your region profile here}",
            color=0x00D9FF,
        )
        embed.add_field(
    ...
    async def give_role(self, msg, roles, divs):
        newb_role = [x for x in roles if x.name.lower() == "newb"][
            0
        ]  # Gets the newb role for removal later
