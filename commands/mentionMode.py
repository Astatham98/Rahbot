...
class MentionMode(BaseCommand):
    def __init__(self):
        description = (
            "Toggles the mention mode. 0 (default) for recommended teams, 1 for an embed of an ordered "
            ...
            if len(params) == 1:
                if int(params[0]) not in [0, 1]:
                    ...
                    await message.channel.send(
                        "Mention mode is now set to {}.".format(settings.MENTION_MODE)
                    )
            elif len(params) == 0:
                # If no params are given return the current mention mode
                await message.channel.send(
                    "Mention mode is currently set to {}.".format(settings.MENTION_MODE)
                )
            else:
