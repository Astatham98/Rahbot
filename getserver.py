...
def get_key():
    ...
    return (
        os.environ.get("SERVERME_KEY")
        if os.environ.get("SERVERME_KEY") is not None
        else config.get("KEYS", "SERVERME_KEY")
    )
...
def get_server(map, loc):
    ...
    response = requests.get(
        "https://direct.{}serveme.tf/api/reservations/new?api_key={}".format(
            prefix, api_key
        ),
        verify=False,
    )
...
def get_map(map):
    maps = {
        "process": "cp_process_f12",
        "sunshine": "cp_sunshine",
        "snakewater": "cp_snakewater_final1",
        "metalworks": "cp_metalworks_f7",
        "gullywash": "cp_gullywash_f9",
        "reckoner": "cp_reckoner_rc6",
        "granary": "cp_granary_pro_rc17a3",
        "prolands": "cp_prolands_rc2ta",
        "bball_skyfall": "ctf_ballin_skyfall",
        "ultiduo_baloo": "ultiduo_baloo_v2",
        "subbase": "cp_subbase_b3a",
        "sultry": "cp_sultry_b8a",
        "product": "koth_product_final",
        "bagel": "koth_bagel_rc12",
        "clearcut": "koth_clearcut_b15d",
    }
    return maps.get(map.lower(), "cp_process_f12")
...
def get_config(region, map):
    maps_configs = {
        ...
        "na": {"cp": 105, "koth": 106, "ctf": 12, "ultiduo": 11},
    }
...
def get_eu_server(servers, loc):
    possible_servers = [
        x for x in servers if x["flag"] == loc and "Anti-DDoS" in x["name"]
    ]
    possible_servers = (
        possible_servers
        if len(possible_servers) > 0
        else [x for x in servers if x["flag"] == loc]
    )
    return possible_servers[0]["id"]


def get_na_server(servers, loc):
    ip_region = {"chi": "nfo-chicago", "ks": "ks", "la": "la"}
